...
class InterModelQM:
	...
	def get_closest_point(self, desired_vals, constraints={}, max_drop_exp_error=-1, skip_list = []):
		...
		return closest_point, match_index, min_val
	...
	def interpolate(self,desired_val_dict,constraints, top_k=5):
		...
		# This loop runs until either the algorithm finds a point that is close enough to the user's desires or until
		...
		while(True):
			...
			if should_skip_optim_rate and should_skip_optim_size and should_skip_optim_constraints \
				...
				start_positions.append(start_pos)
#				return results
		experimental_points = len(all_results)
		for i, idx in enumerate(closest_indices[experimental_points:]):
			result = self.optimize(idx, start_positions[i+experimental_points], norm_constraints)
			all_results.append(result)
		return(all_results)

	def optimize(self, closest_index, start_pos, norm_constraints):
		with open("InterResults_index%d.csv" % closest_index,"w") as f:
			...
			if "generation_rate" not in self.desired_vals_global:
				des_rate = "-1"
			else:
				des_rate = str(self.desired_vals_global["generation_rate"])

			if "droplet_size" not in self.desired_vals_global:
				des_size = "-1"
			else:
				des_size = str(self.desired_vals_global["droplet_size"])
		...
		output_string += "," + str(self.desired_vals_global["droplet_size"])
